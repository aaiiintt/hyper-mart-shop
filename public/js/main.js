...
const scaleTitle = () => {
    window.requestAnimationFrame(() => {
        ...
        const container = title === null || title === void 0 ? void 0 : title.parentElement;
        if (!title || !container) {
            return;
        }
...
const resetAllProducts = () => {
    productList.querySelectorAll('.expanded-content').forEach((content) => {
        content.classList.add('hidden');
    });
    productList.querySelectorAll('.product-row').forEach((rowElement) => {
        rowElement.classList.remove('bg-[#FF00FF]');
    });
    productList.querySelectorAll('.toggle-icon').forEach((iconElement) => {
...
const createProduct = (product) => {
    ...
    price.textContent = (_a = product.price) !== null && _a !== void 0 ? _a : 'INQUIRE';
...
const isProductRecord = (value) => typeof value === 'object' && value !== null;
